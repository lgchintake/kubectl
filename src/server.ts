...
io.on('connection', (socket) => {
    socket.on('list', async (msg, callback) => {
        var data = await execute("kubectl get pods -n axcards-dev");
        data = data.split('\n');
        var finalData = [];
    ...
    socket.on('delete', async (podname, callback) => {
        var data = await execute("kubectl delete pod " + podname + " -n axcards-dev");
        callback({ data: data });
    });

    socket.on('logs', async (podName, callback) => {
        var serviceName = podName.substring(0, podName.lastIndexOf('-', podName.lastIndexOf('-') - 1));
        var data = await execute("kubectl logs " + podName + " -c " + serviceName + " -n axcards-dev");
        callback({ data: data });
    })
});
